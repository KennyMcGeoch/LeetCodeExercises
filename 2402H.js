 ...
 * @param {number[][]} meetings
 * @return {number}
 * Runtime: 250 ms, faster than 65.43% of JavaScript online submissions for Meeting Rooms III.
 * Memory Usage: 82 MB, less than 61.73% of JavaScript online submissions for Meeting Rooms III.
 */
var mostBooked = function(n, meetings) {
    
    meetings.sort((a,b)=>a[0]-b[0])
    
    let roomUses = new Array(n).fill(0)
    let times = new Array(n).fill(0)
    
    for (let i=0; i<meetings.length; i++){
        emptyRoom(meetings[i])
    }
    ...
    let target = Math.max(...roomUses)
    
    for (let i=0; i<roomUses.length; i++) if (roomUses[i] === target) return i
